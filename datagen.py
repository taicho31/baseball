...
def get_top_links(date):
    params = {"date": date}
    schedule_page = requests.get(
        "https://baseball.yahoo.co.jp/npb/schedule", params=params
    )
...
def data_generate(URL, debug=False):
    ...
    # 　選手名を修正
    df["選手名"] = df["選手名"].apply(
        lambda x: x.split(":")[1].split(".")[0].split(team)[0]
    )
...
if __name__ == "__main__":
    ...
    for game_id, game_link in enumerate(game_links):
        ...
        # スタメンを抽出
        data = soup.find_all(
            "td", class_="bb-splitsTable__data bb-splitsTable__data--text"
        )
        data = [i.text.split("\n")[1] for i in data][:20]
